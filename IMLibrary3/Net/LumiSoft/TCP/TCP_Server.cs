...
//using IMLibrary3.Net.Log;

namespace IMLibrary3.Net.TCP
{    
    /// <summary>
    ...
    public class TCP_Server<T> : IDisposable where T : TCP_ServerSession,new()
    {
        ...
        private int                                      m_SessionIdleTimeout   = 60;
        private int                                      m_SessionAuthenticateTime = 5;//认证时间要求
        ...
        private void m_pTimer_IdleTimeout_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                foreach (T session in this.Sessions.ToArray())
                {
                    try
                    {
                        if (!session.IsAuthenticated)//如果未认证
                            if (DateTime.Now > session.ConnectTime.AddSeconds(m_SessionAuthenticateTime))
                            {
                                session.TcpStream.WriteLine("AuthenticateTimeout!");
                                session.OnTimeoutI();
                        ...
                        if (DateTime.Now >session.TcpStream.LastActivity.AddSeconds(m_SessionIdleTimeout))//如果超时
                        {
                            session.TcpStream.WriteLine("IdleTimeout!");
                            session.OnTimeoutI();
                            // Session didn't dispose itself, so dispose it.
                            if (!session.IsDisposed)
                            {
        ...
        private void m_pTimer_IsNotAuthenticatedTimeout_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                foreach (T session in this.Sessions.ToArray())
        ...
        public void Start()
        {
           ...
           //m_pTimer_IsNotAuthenticatedTimeout = new TimerEx(10000, true);
           // m_pTimer_IsNotAuthenticatedTimeout.Elapsed += new System.Timers.ElapsedEventHandler(m_pTimer_IsNotAuthenticatedTimeout_Elapsed);
           // m_pTimer_IsNotAuthenticatedTimeout.Enabled = true;
        ...
        /// 设置认证超时秒数
        /// </summary>
        public int SessionAuthenticateTime
        {
            get
            {
                if (m_IsDisposed)
                ...
                return m_SessionAuthenticateTime;
            }
            ...
            set
            {
                ...
                m_SessionAuthenticateTime = value;
            }
        }
